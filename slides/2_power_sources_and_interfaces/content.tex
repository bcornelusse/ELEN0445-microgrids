...
\begin{frame}{6-pulse rectifier (3-phase source)} 
  \centering
  \href{https://youtu.be/WVI8Z7p_rdY}{\includegraphics[width=0.6\textwidth]{images/video_6pulse_rectifier.png}}
\end{frame}
...
\begin{frame}{Basic block: two switches}
  \begin{center}
...
\begin{frame}{How an inverter works}
  \centering
  \href{https://youtu.be/qVeERT4nyz8}{\includegraphics[width=0.6\textwidth]{images/video_inverter.png}}
\end{frame}


\begin{frame}{6-pulse inverter (3-phase)}
  \centering
  \href{https://youtu.be/jOquS6uqpeY}{\includegraphics[width=0.6\textwidth]{images/video_6pulse_inverter.png}}
\end{frame}
...
E_{w}=\frac{1}{2} m v^{2}
$$
As power is the time derivative of energy, we have, assuming the speed is constant:
$$
...
Only a fraction of the wind power is harvested by the blades. Actually, the energy harvested is a function of the \textbf{speed of the air that enters the blades}, $v_{u}$, and \textbf{speed of the air that leaves the blades}, $v_{d}$ :
$P_{b}=\frac{1}{2} \frac{d m}{d t}\left(v_{u}^{2}-v_{d}^{2}\right)$

Approximating $\frac{d m}{d t}$ by $\rho A \frac{v_{u}-v_{d}}{2}$ and defining the coefficient $\lambda_{w}$ as
$
...
\begin{center}
  \href{https://youtu.be/JJr4PIuQp2w}{\includegraphics[width=0.5\textwidth]{images/video_wind.jpg}}
\end{center}
...
\begin{itemize}
  \item the output of the generator goes through a DC conversion stage (rectifier if AC generator, DC-DC if DC generator) to cope with wind speed variations
...
\begin{columns}
    ...
    \begin{column}{0.5\textwidth}
      \begin{center}        
      \href{https://youtu.be/L_q6LRgKpTw}{\includegraphics[width=0.8\textwidth]{images/video_PV.jpg}}
      \end{center}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{itemize}
  ...
  \item The open-circuit voltage is approximately equal to the voltage across the diode
  \item Both are a function of irradiance and temperature
\end{itemize}
...
Hence a shadowed or damaged panel can impact the whole array. In practice, PV panels arrangement is a mix of series and parallel connections. This trade-off is also impacted by the number and types of power electronics equipment that a particular configuration requires.
...
\begin{frame}[allowframebreaks]{Maximum power point tracking (MPPT)}
  ...
  \begin{column}{0.45\textwidth}
    ...
    \begin{enumerate}
      ...
      \item $R_{M P P}$ is a function of irradiance and temperature.
    \end{enumerate}
  \end{column}
  \begin{column}{0.45\textwidth}
...
\begin{frame}{More on MPPT algorithms}
  \centering
  \href{https://youtu.be/0ItjKs7aJFM}{\includegraphics[width=0.6\textwidth]{images/video_MPPT.png}}
\end{frame}
...
\begin{itemize}
  ...
  \item the membrane is made of Nafion
\end{itemize}
...
\begin{itemize}
  \item Fuel cells have an MPP of operation that corresponds to a cell output voltage of approximately 0.4 V
...
A microturbine thus requires a rectifier (+ inverter if connected to an AC grid).
