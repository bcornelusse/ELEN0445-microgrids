...
\begin{frame}{Questions addressed}
    ...
    \begin{itemize}
      \item Should I invest in storage and local generation?
      \item If yes, how to size these devices?
      \item Which grid connection should I contract?
      \item What if I have several technological options?
      \item How will I manage the day-to-day operations?
      \item Is my project financially viable?
    \end{itemize}
\end{frame}

\begin{frame}{The sizing problem addresses these questions as an extension of the operational planning problem}
    Main steps: 
    \begin{enumerate}
      \item create the options and let their capacities (sizes) vary
      \item redesign the objective function properly
      \item extend the time horizon to capture the main events the system will face.
    \end{enumerate}
\end{frame}

\begin{frame}{What's in these slides?}

    This presentation summarizes a (stochastic mixed-integer)\footnote{Stochastic means we will consider several scenarios and attempt to have a robust design. Mixed-Integer means some decisions will be binary, essentially.} \textbf{linear optimization formulation} of this sizing problem, emphasizing:
    \begin{itemize}
        \item generation, converter, and battery storage system \textbf{degradation}
        \item corresponding necessary \textbf{reinvestments} along the life of the project
        \item \textbf{connection costs} to the public grid.
    \end{itemize}
    
    The resolution of this large optimization problem is facilitated by the use of \textbf{representative days}. 
...
\begin{frame}[allowframebreaks]{Net Present Value (NPV)}
  \begin{itemize}
    ...
    \end{align} 
    \item The numerator terms depend on the microgrid \textbf{sizing decisions} $c$ and \textbf{control decisions} $a$ (we will come back to this later), representing annual costs. 
    \item $I$ stands for \textbf{(re)investment costs} $R$ for \textbf{revenues} and $O$ for overall \textbf{operating costs}. 
    \item The present value of the cash flow is obtained by using a \textbf{discount factor} $d$ applied over the \textbf{microgrid lifetime} $Y$. 
    ...
    \item A \textbf{salvage value} may be taken into account.
  \end{itemize}
\end{frame}
...
\begin{frame}{NPV Example: Solar Farm Project}
  \begin{itemize}
    ...
    \item Remark: totally made up numbers
  \end{itemize}
\end{frame}
...
\begin{frame}{NPV Calculation}
  \begin{align*}
    NPV & = \sum_{y=0}^{Y} \frac{-I_y+R_y-O_y}{(1+d)^y} \\
    &= -\text{\texteuro}1,000,000 + \frac{\text{\texteuro}230,000}{(1.08)^1} + \frac{\text{\texteuro}270,000}{(1.08)^2} + \frac{\text{\texteuro}310,000}{(1.08)^3} + \frac{\text{\texteuro}323,000}{(1.08)^4} + \frac{\text{\texteuro}445,000}{(1.08)^5}\\
    & \approx \text{\texteuro} 231,000 
  \end{align*}
...
\toprule
Years & Num & \multicolumn{4}{c}{Discount rate} \\
\cmidrule(lr){3-6}
...
The sizing problem can be seen as an operational planning problem extended over the lifetime of the project:
\begin{align*}
...
\end{align*}
\begin{itemize}
  \item We let device capacities vary (instead of fixed parameters) and account for their CAPEX in the NPV.
  \item We optimize NPV to account for the time value of money, and consider several years to scale OPEX w.r.t. CAPEX.
  \item A variation of the consumption over the years can also be included.
\end{itemize}
