...
\includegraphics[width=0.8\textwidth, center]{4f5abcf0-f301-43b5-aaf0-9e34a9c79ca8-03_1374_2361_286_484}\\
\end{frame} 
\begin{frame}{L1}
    \includegraphics[width=0.8\textwidth, center]{4f5abcf0-f301-43b5-aaf0-9e34a9c79ca8-04_678_2372_24_478}\\
\end{frame}
\begin{frame} {L2}
    \includegraphics[width=0.8\textwidth, center]{4f5abcf0-f301-43b5-aaf0-9e34a9c79ca8-04_446_2337_695_492}
\end{frame}
\begin{frame} {L3}
    \includegraphics[width=0.8\textwidth, center]{4f5abcf0-f301-43b5-aaf0-9e34a9c79ca8-04_408_2336_1185_489}\\
\end{frame} 
\begin{frame}{Aggregated}
    \includegraphics[width=0.8\textwidth, center]{4f5abcf0-f301-43b5-aaf0-9e34a9c79ca8-05_1124_2317_429_510}
\end{frame} 
...
For computational reasons, we discretize the decisions in time
\begin{center}
    \includegraphics[width=0.5\textwidth]{4f5abcf0-f301-43b5-aaf0-9e34a9c79ca8-12_257_1207_1495_1041}
\end{center}
\begin{itemize}
  \item Depending on the decision horizon, we may consider periods of 1 to 15 minutes where decisions are \textbf{assumed constant}, e.g.
  \begin{itemize}
    \item constant production of PV panels, 
    \item constant power for battery charging/discharging
  \end{itemize}
...
\begin{frame}{Operation as a sequential decision-making problem}
  ...
  \begin{column}{0.45\textwidth}
    \begin{center}
        \includegraphics[width=\textwidth]{state_trajectory.pdf}
    \end{center}
  \end{column}
\end{columns}
...
\begin{frame}{Given a criterion, we can compute an open-loop sequence of actions $\boldsymbol{u}^{*}$ to drive the system }
...
\begin{itemize}
  ...
  \item This can be mitigated by a receding horizon approach
\end{itemize}

\end{frame} \begin{frame}{Receding horizon approach}
Example: demand variation or restriction on $x$ or $u$ are observed at time $1$ \\
\vspace{0.5cm}
  ...
  \begin{column}{0.5\textwidth}
    ...
    \begin{itemize}
      \item after applying the first decision (e.g. $u_0$), you are at time $1$
      \item update the sequence of control actions
      \begin{enumerate}
        \item Re-estimate the parameters (demand, availabilities) and the actual state $x_1$ reached.
        \item Re-solve the open-loop formulation ($u', x'$) until $T+1$
      \end{enumerate}
      \item ...
    \end{itemize}
  ...
  \begin{column}{0.45\textwidth}
    \begin{center}
        \includegraphics[width=\textwidth]{receding_horizon.pdf}\\
    \end{center}
  \end{column}
\end{columns}
...
\begin{itemize}
  ...
  \item In operation, scheduled maintenance is just extra constraints or data updates (e.g. PV is out)
...
\begin{frame}{The invoice}
 (In Belgium), the invoice of a user connected to a distribution network is composed of:
  \begin{itemize}
...
\end{itemize}
This varies a lot as a function of the type of connection (i.e. of the voltage level and overall consumption)

\end{frame} 

\begin{frame}{Minimize the energy purchase cost}
The microgrid is subject to a dynamic price signal. At time $t$, it can buy electricity at a price $p_{t}$.
...
\end{itemize}
Note: for now, we consider that
\begin{itemize}
  \item the microgrid cannot influence the price
  \item there is no imbalance penalty
\end{itemize}

Depending on the decision time, we can not always assume the price is known in advance.
...
\begin{itemize}
  ...
  \item Batteries suffer from the same problem. However, this depends a lot on the type of battery technology.
\end{itemize}
...
\end{frame} \begin{frame}{All these objectives are (most of the time) conflicting}
For instance:
\begin{itemize}
...
\end{frame} \begin{frame}{Loads can be subdivided into 5 categories}
\begin{itemize}
