...
\begin{columns}
    \begin{column}{0.5\textwidth}
        ...
        \begin{tikzpicture}[node distance=2cm, auto, >=stealth, thick, scale=0.8, every node/.style={scale=0.8}]
            % Nodes
            \node[] (grid) {\includegraphics[width=1cm]{images/utility-pole.png}};
            \node[below=0.2cm of grid, font=\scriptsize] {Public Grid};
            \node[circle, draw, right=2cm of grid] (sum) {\Large $+$};
            \node[right=1.5cm of sum, inner sep=5pt] (house) {\includegraphics[width=1cm]{images/home.png}}; 
            \node[below=1.5cm of sum] (pv) {\includegraphics[width=1cm]{images/PV.png}};
            ...
            \draw<2>[->] (grid) -- node[above] {1.4 kW} (sum);
            % State 3: PV drops, House 0.7, EV 7 -> Need 6.7kW from grid (Import)
            \draw<3>[->] (grid) -- node[above] {6.7 kW} (sum);
            ...
            \node<1->[above=1.5cm of sum] (ev) {\includegraphics[width=1cm]{images/electric-car.png}};
...
\begin{frame} {Sizing depends on the operational planning policy}
    ...
    \begin{columns}
        \begin{column}{0.38\textwidth}
            Example: 
            \begin{itemize}
                \item 2 EVs and you need to choose how many Amps to take from the grid
                \item Can charge slow or fast
                \item Charge them simultaneously or one after the other.
            \end{itemize}
        \end{column}
        \begin{column}{0.6\textwidth}
            \includegraphics[width=\textwidth]{images/2EVs.pdf}
        \end{column}
    \end{columns}
    \vspace{5mm}
    \textbf{Your ability to optimize charges will largely impact the grid connection.}
    \begin{itemize}
        \item Good forecasts of prices, generation, and consumption required
    \end{itemize}
\end{frame}
